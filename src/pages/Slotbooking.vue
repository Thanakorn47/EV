      ...
      <section class="booking-history">
        ...
        <div v-if="bookingHistory.length > 0">
          ...
          >
            ...
            <button
              @click="cancelBooking(booking.bookingDId, booking.chargerDId)"
    ...
    <aside class="available-slots">
      ...
      <div v-for="station in stations" :key="station.id" class="station">
        ...
        >
          ...
          <button
            v-if="charger.available"
            @click="bookSlot(charger.documentId)"
            class="book-button"
          >
            Book Now
          </button>
...
export default {
  ...
  methods: {
    ...
    async cancelBooking(bookingID, chargerId) {
      await axios.delete(
        `https://strapi-sever-ev.onrender.com/api/reservations/${bookingID}`
      );
      await axios.put(
        `https://strapi-sever-ev.onrender.com/api/chargers/${chargerId}`,
        {
          data: {
            available: true,
          },
